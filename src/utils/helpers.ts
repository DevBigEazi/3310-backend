...
export function getDayId(date = new Date()): number {
  ...
  return Math.floor((currentTimestamp - GENESIS_TIMESTAMP) / SECONDS_PER_DAY) + 1;
}
...
export async function checkAndRewardReferrer(playerAddress: string, score: number): Promise<void> {
  try {
    ...
    const gameSession = await GameSession.findOne({ playerAddress, dayId: currentDay });
    if (!gameSession) {
      return;
    }
    ...
    if (gameSession.dailyAccumulatedScore >= 50) {
      ...
      // Add 50 points to referrer's daily accumulated score
      ...
      if (referrerGameSession) {
        ...
        // Create a new game session for the referrer if they don't have one for this day
